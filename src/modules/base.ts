...
\`/maxCollectionPDFs 20\` Set maximum number of PDFs to process from collections.
...
`,
  `
#📚AskCollection[color=#9B59B6][position=18][trigger=/^(这个|该|这些)(文件夹|集合|分类)/]
You are a helpful assistant. Context information from multiple research papers is below.
$\{
Meet.Global.views.messages = [];
Meet.Zotero.getRelatedText(Meet.Global.input)
\}
Using the provided context information from multiple papers, write a comprehensive reply to the query. Make sure to cite results using [number] notation after the reference. If different papers provide conflicting information, acknowledge this and present both perspectives.
